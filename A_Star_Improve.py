...
class A_Star:
    ...
    #H函数，估计从当前节点到目标节点的最小距离从而计算出从起点到目标点的距离
    def get_dist(self, i):
        # F = G + H
    ...
    def extend_round(self, p):
        ...
        if self.e_y < p.y:
            ...
            dir_y = 1
        #剪枝优化，如果往目标方向节点扩展的节点可达，则直接使用扩展节点
        if self.is_valid_coord(p.x + dir_x, p.y + dir_y):
            node = Node_Elem(p, p.x + dir_x, p.y + dir_y, p.dist + self.get_cost(
                p.x, p.y, p.x + dir_x, p.y + dir_y))
            # 新节点在关闭列表，则忽略
...
class main_func():
    ...
    def input_tm(self):
        ...
        try:
            ...
            num = int(f.readline())
            # 输入起始终止坐标:
            S_x, S_y = map(int, f.readline().split())
            E_x, E_y = map(int, f.readline().split())
            for i in range(x_len + 2):
                ...
                for j in range(y_len + 2):
                    ...
                    if i == 0 or i == x_len + 1 or j == 0 or j == y_len + 1:
                        ...
                        if (i != S_x or j != S_y) and (i != E_x or j != E_y):
...
def find_path():
    ...
    e_x, e_y = get_end_XY()
    a_star = A_Star(s_x, s_y, e_x, e_y,len(tm),len(tm[0]))
    a_star.find_path()
